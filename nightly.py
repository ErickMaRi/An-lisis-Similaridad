...
def plot_interactive_networkx(similarity_matrix, df, id_to_query_map, paper_ids):
    """
    TODO: Cuando la función está en el modo coloreado por cantidad de conexiones, no produce
    el tooltip con el nombre del paper, además desearía tener una sola figura en la que el
    color del nodo es determinado por la cantidad de conexiones y la forma definida por
    el término de búsqueda asociado al paper.
